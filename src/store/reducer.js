...
import {
    ADD_BOARD, SELECT_BOARD, RENAME_BOARD, DELETE_BOARD, LOAD_MARKDOWN_DATA,
    ADD_PHASE, SELECT_PHASE, RENAME_PHASE, DELETE_PHASE,
    SET_TASKS_FOR_ACTIVE_BOARD, ADD_TASK_TO_COLUMN, DELETE_TASK, UPDATE_TASK_DUE_DATE,
  ...
  export const kanbanReducer = (state, action) => {
    switch (action.type) {
      ...
      case ADD_BOARD: {
        ...
        const newBoard = {
          id: newBoardId,
          name: action.payload.boardName.trim() || `Board ${state.boards.length + 1}`,
      ...
      case SELECT_BOARD: {
        const boardToSelect = state.boards.find(b => b.id === action.payload.boardId);
        if (boardToSelect) {
          // Ensure the board has phases, then select the first one
          const firstPhaseId = (boardToSelect.phases && boardToSelect.phases.length > 0)
          ...
          return { ...state, activeBoardId: action.payload.boardId, activePhaseId: firstPhaseId };
        }
        return state;
      }
      ...
      case RENAME_BOARD: {
        ...
        return {
          ...
          boards: state.boards.map(b =>
            b.id === action.payload.boardId ? { ...b, name: action.payload.newName.trim() } : b
          ),
        };
      }
      ...
      case DELETE_BOARD: {
        ...
        const remainingBoards = state.boards.filter(b => b.id !== action.payload.boardId);
        ...
        if (state.activeBoardId === action.payload.boardId) {
          ...
          if (newActiveBoardId) {
            const newActiveBoardData = remainingBoards.find(b => b.id === newActiveBoardId);
            newActivePhaseId = (newActiveBoardData?.phases && newActiveBoardData.phases.length > 0)
      ...
      case LOAD_MARKDOWN_DATA: {
        ...
        let { phases: parsedPhases, tasks: parsedTasks } = parseMarkdown(action.payload.markdownText);
        ...
        if (parsedPhases.length > 0) {
          ...
          parsedTasks = parsedTasks.map(task => ({ ...task, phaseId: task.phaseId || newActivePhaseId }));
        }
  
        return {
          ...state,
          boards: state.boards.map(b =>
            b.id === state.activeBoardId ? { ...b, phases: parsedPhases, tasks: parsedTasks } : b
          ),
          activePhaseId: newActivePhaseId,
        };
      }
      ...
      case ADD_PHASE: {
        if (!state.activeBoardId) { alert("No active board selected."); return state; }
        if (!action.payload.phaseName.trim()) { alert("Phase name cannot be empty."); return state; }
        const newPhase = { id: generateUniqueId('phase'), name: action.payload.phaseName.trim() };
      ...
      case RENAME_PHASE: {
        ...
        if (!state.activeBoardId) { alert("No active board selected."); return state; }
        if (!action.payload.newName.trim()) { alert("Phase name cannot be empty."); return state; }
        return {
          ...state,
            ...
            board.id === state.activeBoardId
              ? {
                  ...
                  phases: (board.phases || []).map(phase =>
                    phase.id === action.payload.phaseId ? { ...phase, name: action.payload.newName.trim() } : phase
                  ),
                }
              : board
          ),
      ...
      case DELETE_PHASE: {
        ...
        const newBoards = state.boards.map(board => {
          if (board.id === state.activeBoardId) {
            const remainingPhases = (board.phases || []).filter(phase => phase.id !== action.payload.phaseIdToDelete);
            if (state.activePhaseId === action.payload.phaseIdToDelete) {
              newActivePhaseId = (remainingPhases.length > 0) ? remainingPhases[0].id : null; // Default to first remaining
            }
            return {
              ...board,
              phases: remainingPhases,
              tasks: (board.tasks || []).map(task =>
                task.phaseId === action.payload.phaseIdToDelete ? { ...task, phaseId: null } : task // Unassign tasks from deleted phase
              ),
            };
          }
          return board;
          ...
          return {
            ...
            boards: state.boards.map(board => {
              if (board.id === state.activeBoardId) {
                ...
                let updatedTasks = typeof action.payload.tasksOrUpdater === 'function' 
      ...
      case ADD_TASK_TO_COLUMN: {
          ...
          if (!phaseForNewTask || !currentBoard.phases.some(p => p.id === phaseForNewTask)) {
          ...
          if (currentBoard.phases.length === 1 && currentBoard.phases[0].id === phaseForNewTask && state.activePhaseId !== phaseForNewTask) {
          ...
          return {
            ...
            boards: state.boards.map(b =>
              b.id === state.activeBoardId
                ? { ...b, tasks: b.tasks.filter(t => t.id !== action.payload.taskId) }
                : b
            ),
          };
      }
            ...
            boards: state.boards.map(b =>
              b.id === state.activeBoardId
                ? {
                    ...b,
                    tasks: b.tasks.map(t =>
                      t.id === action.payload.taskId ? { ...t, dueDate: action.payload.newDueDate } : t
                    ),
                  }
                : b
            ),
