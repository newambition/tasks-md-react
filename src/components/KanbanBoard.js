      ...
      if (activeContainerId === overContainerId) {
        ...
        if (overTaskIndexInFullList !== -1) {
          ...
          if (lastIndexOfThisColumn !== -1)
            ...
            // Column became empty, or was always empty except for this task
      ...
      <DragOverlay dropAnimation={null}>
        {activeTaskId && activeTaskForOverlay ? (
          <TaskCard
            ...
            onUpdateTaskDueDate={() => {}}
            onUpdateTask={() => {}} // Overlay card is non-interactive
            isExpanded={false} // Overlay should never be expanded
            onExpand={() => {}} // Overlay card is non-interactive
            // Pass label props so labels are visible during drag
            boardLabels={boardLabels}
            onCreateLabel={() => {}} // Overlay card is non-interactive
            onUpdateLabel={() => {}} // Overlay card is non-interactive
            onDeleteLabel={() => {}} // Overlay card is non-interactive
          />
        ) : null}
      </DragOverlay>
    </DndContext>
