...
export const getInitialData = () => {
          ...
          boards: parsedData.boards.map((board) => {
            const normalizedBoard = {
              ...
              phases: Array.isArray(board.phases)
                ? board.phases.map((p) => ({
                    id: p.id || generateUniqueId("phase"),
              ...
              tasks: Array.isArray(board.tasks)
                ? board.tasks
                    .filter(
                      (t) =>
                        t && t.id && t.status && COLUMN_IDS.includes(t.status)
                    )
                    .map((t) => ({
                      ...t,
                      phaseId: t.phaseId === undefined ? null : t.phaseId,
                    }))
                : [],
              labels: Array.isArray(board.labels) ? board.labels : [], // Normalize labels
            };
      ...
      boards: [
        {
          ...
          labels: [], // Add labels to default board
          tasks: [
            {
  ...
  let activeBoard = loadedState.boards.find(
    (b) => b.id === loadedState.activeBoardId
  );
    ...
    loadedState.boards = [
      {
        id: defaultBoardId,
        name: DEFAULT_BOARD_NAME,
        phases: [{ id: defaultPhaseId, name: "General" }],
        tasks: [],
        labels: [],
      },
    ];
  ...
  if (activeBoard && activeBoard.phases.length > 0) {
    const phaseExists = activeBoard.phases.some(
      (p) => p.id === loadedState.activePhaseId
    );
    if (!phaseExists) {
